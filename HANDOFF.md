...
- **Column-level grant on `profiles`.** The migration does `REVOKE UPDATE ON profiles FROM authenticated` and then `GRANT UPDATE(display_name) ON profiles TO authenticated`. Only `display_name` is updatable via the anon-key client; any future profile field that users should be able to edit needs an explicit `GRANT UPDATE(<col>)`. Admin updates to other columns (e.g. `is_admin`) must go through the service-role client.
